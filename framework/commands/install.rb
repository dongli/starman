...
EOS
    ...
    @parser.on '--clean-path', 'Clean PATH environment variable.' do |clean_path|
      @@args[:clean_path] = true
    end
  ...
  def run
    ...
    if CommandParser.args[:clean_path]
      clean_path = []
    ...
    PackageLoader.loaded_packages.each do |name, package|
      ...
      elsif res.class == Array and res[0] == :different_version_installed
